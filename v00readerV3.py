...
class V00ReaderGUI:
    def __init__(self, root):
        ...
        self.view_max_depth = None   # meters, None = full depth
        ...
        tk.Button( self.input_frame, text="Depth Crop", command=self.depth_crop_dialog, bg="#E1F5FE" ).grid(row=0, column=14, padx=5, pady=8)
        tk.Button( self.input_frame, text="Depth View", command=self.depth_view_dialog, bg="#E1F5FE" ).grid(row=1, column=14, padx=5, pady=8)
        
        # ================= DECLUTTERING BUTTONS (MS, SVD, RNMF) =================
        tk.Button(self.input_frame, text="MS", command=self.ms_declutter_dialog, bg="#FFCCBC").grid(row=0, column=15, padx=4, pady=5)
        tk.Button(self.input_frame, text="SVD", command=self.svd_declutter_dialog, bg="#FFCCBC").grid(row=1, column=15, padx=4, pady=5)
        tk.Button(self.input_frame, text="RNMF", command=self.rnmf_declutter_dialog, bg="#FFCCBC").grid(row=2, column=15, padx=4, pady=6)
    ...
    def _render_wiggle(self, trace_skip=1, amplitude_scale=1.0, clip_percent=100.0,
        ...
        if self.loader.xyz is not None and len(self.loader.xyz) > 0:
            x_all = self.loader.xyz['X'].values
            if len(x_all) != n_traces:
                x_all = np.linspace(x_all[0], x_all[-1], n_traces)
        else:
            # Use cell spacing if available
            if self.loader._x_cell_m is not None:
                x_all = np.arange(n_traces) * self.loader._x_cell_m
            else:
